...
{
    public partial class Building : System.Web.UI.MasterPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            //每次点击母版页导航栏时调用，将矿物矿场cookie增值保存到数据库
            //为了不弄混，决定手动赋值
            HttpCookie player_id_cookie = Request.Cookies["player_id"];
            int player_id = Convert.ToInt32(player_id_cookie.Value.ToString());
            Sqlconn sqlconn = new Sqlconn();
            DataTable resource1 = sqlconn.SelectResourceSignin(player_id);
            //-----------------------矿物总值----------------------------------
            ...
            //------------------------矿物总值-----------------------------------
            int al_r_num = Convert.ToInt32(resource1.Rows[0][0].ToString());
            int fe_r_num = Convert.ToInt32(resource1.Rows[0][1].ToString());
            int cu_r_num = Convert.ToInt32(resource1.Rows[0][2].ToString());
            int pb_r_num = Convert.ToInt32(resource1.Rows[0][3].ToString());
            int sn_r_num = Convert.ToInt32(resource1.Rows[0][4].ToString());
            int ti_r_num = Convert.ToInt32(resource1.Rows[0][5].ToString());
            int cr_r_num = Convert.ToInt32(resource1.Rows[0][6].ToString());
            int ni_r_num = Convert.ToInt32(resource1.Rows[0][7].ToString());
            //---------------------------矿物变动值-------------------------------
